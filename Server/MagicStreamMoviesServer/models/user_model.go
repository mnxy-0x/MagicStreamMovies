...
// user inputs in login Form [no need to bson in this situation]
type UserLogin struct {
	Email    string `json:"email" validate:"required,email"`
	Password string `json:"password" validate:"required,min=6"`
}

// DTO - sent from server to front-end as response of user-login operation(no DB operation; so no bson needed)
